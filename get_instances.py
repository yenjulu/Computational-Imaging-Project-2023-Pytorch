...
def get_dataset(dataset_name, dataset_params, mode, verbose=True):
    ...
    if dataset_name == 'modl_fastmri_dataset':
        from datasets.modl_dataset import modl_fastmri_dataset
        dataset = modl_fastmri_dataset(mode=mode, **dataset_params)
        
    if dataset_name == 'modl_fastmri_datasetx4':
        from datasets.modl_dataset import modl_fastmri_datasetx4
        dataset = modl_fastmri_datasetx4(mode=mode, **dataset_params)

    if dataset_name == 'ssdu_fastmri_dataset_modl':
        from datasets.ssdu_dataset import ssdu_dataset_modl
        dataset = ssdu_dataset_modl(mode=mode, **dataset_params)

    if dataset_name == 'ssdu_fastmri_dataset_torch':
        from datasets.ssdu_dataset import ssdu_dataset_torch
        dataset = ssdu_dataset_torch(mode=mode, **dataset_params)
...
def get_model(model_name, model_params, device):
    ...
    elif model_name == 'base_modl_ssdu':
        from models.modl import MoDL_ssdu
        model = MoDL_ssdu(**model_params)
    elif model_name == 'base_modl_gd':
        from models.modl import MoDL_gd
        model = MoDL_gd(**model_params)
    elif model_name == 'base_varnet':
        from models.varnet import VarNet
        model = VarNet(**model_params)
    elif model_name == 'base_varnet_ssdu':
        from models.varnet import VarNet_ssdu
        model = VarNet_ssdu(**model_params)
    elif model_name == 'transformer_modl':
        from models.modl import MoDL_Transformer
        model = MoDL_Transformer(**model_params)
    elif model_name == 'transformer_varnet':
        from models.varnet import VarNet_Transformer
        model = VarNet_Transformer(**model_params)
    elif model_name == 'unet_transformer_varnet':
        from models.varnet import VarNet_Unet_Transformer
        model = VarNet_Unet_Transformer(**model_params)
    elif model_name == 'base_UnrollNet':
        from models.UnrollNet_torch import UnrolledNet
        model = UnrolledNet(**model_params)
    elif model_name == 'base_UnrollNet_GD':
        from models.UnrollNet_torch import UnrolledNet_GD
        model = UnrolledNet_GD(**model_params)
...
def get_score_fs(score_names):
    ...
    for score_name in score_names:
        if score_name == 'PSNR':
            ...
            score_f = ssim_batch
        elif score_name == 'PSNR_ssdu':
            from utils_ssdu import getPSNR
            score_f = getPSNR
