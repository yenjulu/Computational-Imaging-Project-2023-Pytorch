...
def sense1(input_kspace, sens_maps, axes=(-2, -1)):
    ...
    image_space = utils_ssdu.ifft(input_kspace, axes=axes, norm=None, unitary_opt=True)
...
class SenseOp():
    ...
    def fwd(self, input) -> torch.Tensor:
        ...
        # input = N_batch, N_y, N_x --> [1, N_batch, N_y, N_x] 
        
        # kfull = fftc(coils, norm='ortho')  # coil here is coil-specific input img 
        ######
        kfull = utils_ssdu.fft_torch(coils, axes=(-2, -1), norm=None, unitary_opt=True)
        ######
    ...
    def adj(self, input) -> torch.Tensor:
        ...
        if self.traj is None:
            ...
            #############
            imask = utils_ssdu.ifft_torch(kmask, axes=(-2, -1), norm=None, unitary_opt=True)
            #############
        else:
            # TODO: Radial sampling
            None
