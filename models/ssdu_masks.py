import numpy as np
from utils_ssdu import index_flatten2nd, find_center_ind
import scipy.io as sio
### no needs to be torch version ###
...
class ssdu_masks():
    ...
    def Gaussian_selection(self, std_scale=4, num_iter=1):
        # nrow, ncol = input_data.shape[0], input_data.shape[1]   # original
        ...
        while count <= int(np.ceil(np.sum(np.abs(input_mask[:])) * self.rho)):
